...
from app.core.numeric_text import numeric_format_anomaly, parse_numeric_value
...
class NumericTextTest(unittest.TestCase):
    ...
    def test_unmatched_numeric_parenthesis_is_review_candidate(self) -> None:
        anomaly = numeric_format_anomaly("621)", unit="건", peer_values=["620", "622"])
        self.assertIsNotNone(anomaly)
        self.assertEqual(anomaly.suggested_value, "621")
        self.assertIsNone(numeric_format_anomaly("1)", unit="", peer_values=[]))
    ...
    def test_dates_and_prose_commas_are_not_numeric_candidates(self) -> None:
        self.assertIsNone(numeric_format_anomaly("1998.4.1.", unit="건"))
        self.assertIsNone(numeric_format_anomaly("47.3(42.4)", unit="%"))
        self.assertIsNone(numeric_format_anomaly("513\n(463)", unit="건"))
        self.assertIsNone(numeric_format_anomaly("257,702\n(123.2%)", unit="개, %"))
        self.assertIsNone(numeric_format_anomaly("’24", unit="억원"))
        self.assertIsNone(
            numeric_format_anomaly(
                "총무처, 공보처, 법제처를 개편",
                unit="개",
